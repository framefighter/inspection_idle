...
pub fn drive_robot(
    ...
    drivable_query: Query<(Entity, &Drivable)>,
    mut rigid_body_query: Query<(&mut RigidBodyForces, &Transform), With<Selected>>,
    parent_query: Query<(&Parent, &Transform)>,
) {
    drivable_query.for_each_mut(|(drivable_entity, drive)| {
